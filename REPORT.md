...
*   **Architecture & Navigation**: The app follows the **MVVM (Model-View-ViewModel)** architectural pattern. From the `MainActivity`, navigation is handled via explicit `Intent` objects. For an immersive launch, the Scan View utilizes `ActivityOptionsCompat.makeSceneTransitionAnimation` with the logo as a shared element, while the other navigation buttons use standard intent transitions.
